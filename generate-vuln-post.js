...
function readPromptTemplate() {
  ...
  const promptName = options.rag !== false ? "threat-blog-post-rag.prompt" : "threat-blog-post.prompt";
  ...
  if (!fs.existsSync(promptPath) && promptName === "threat-blog-post-rag.prompt") {
    console.log("RAG-enabled prompt template not found, falling back to standard template");
  ...
  try {
    ...
    if (mitreData) {
      ...
      return {
        ...
        references: mitreData.references.map(ref => ({ 
          url: ref.url, 
...
async function getMitreCveData(cveId) {
  ...
  try {
    ...
    if (response.data && response.data.cveMetadata) {
      ...
      let description = '';
      const englishDescriptions = data.containers?.cna?.descriptions?.filter(d => d.lang === 'en') || [];
      ...
      if (englishDescriptions.length > 0) {
        ...
        if (englishDescriptions.length > 1) {
          const additionalDescriptions = englishDescriptions.slice(1).map(d => d.value).filter(Boolean);
      ...
      if (metrics && metrics.length > 0) {
        ...
        primaryMetric = metrics.find(m => m.type === 'CVSS v3.1') || 
      ...
      const processed = {
        ...
        recommendations: data.containers?.cna?.solutions?.map(s => s.value).join('\n') || null
      };
...
async function getVulDbData(cveId) {
  ...
  while (retryCount < maxRetries) {
    try {
      console.log(`Checking VulDB for information on ${cveId}... (Attempt ${retryCount + 1}/${maxRetries})`);
      ...
      const response = await axios.get("https://vuldb.com/?rss.recent", {
        ...
        headers: {
          ...
          'User-Agent': process.env.VULDB_USER_AGENT || 'William Zujkowski Blog Vulnerability Analyzer'
        }
      });
      
      // Check if response is valid XML
      if (!response.data || typeof response.data !== 'string' || !response.data.includes('<rss')) {
      ...
      return title.includes(cveId) || description.includes(cveId) || guid.includes(cveId);
    });
    ...
    console.log(`Found ${relevantItems.length} items in VulDB feed related to ${cveId}`);
    ...
    const vuldbData = relevantItems.map((item) => {
      ...
      let severity = null;
      const severityMatch = description.match(/Risk Level:\s*(Critical|High|Medium|Low)/i) ||
      ...
      let cvssScore = null;
      const cvssMatch = description.match(/CVSS Base Score:\s*(\d+(\.\d+)?)/i) ||
      ...
      if (productSection && productSection[1]) {
        ...
        affectedProducts = productSection[1].split(/,|;/).map(p => p.trim()).filter(Boolean);
      }
    ...
    return vuldbData;
      ...
      retryCount++;
      console.error(`Error fetching data from VulDB (attempt ${retryCount}/${maxRetries}):`, error.message);
...
async function getCertCcData(cveId) {
  ...
  try {
    ...
    const relevantEntries = entries.filter((entry) => {
      ...
      return title.includes(cveId) || summary.includes(cveId) || content.includes(cveId);
    });
    ...
    const certData = relevantEntries.map((entry) => {
      ...
      if (highSeverityTerms.some(term => titleLower.includes(term) || summaryLower.includes(term))) {
        severity = "High";
      } else if (mediumSeverityTerms.some(term => titleLower.includes(term) || summaryLower.includes(term))) {
        severity = "Medium";
      } else if (lowSeverityTerms.some(term => titleLower.includes(term) || summaryLower.includes(term))) {
        severity = "Low";
      }
...
async function createInputData(cveId) {
  ...
  if (certCcData && certCcData.length > 0) dataSources.push("CERT Coordination Center (CERT/CC)");
  if (vuldbData && vuldbData.length > 0) dataSources.push("VulDB (Vulnerability Database)");
  if (
    process.env.ALIENVAULT_OTX_ENABLED === "true" &&
    process.env.ALIENVAULT_OTX_API_KEY &&
...
function createMinimalInputData(cveId, fallbackVulnData, mitreCveData, zdiData) {
  ...
  let vulnName = cveId;
  let vulnType = "";
  let mitigationGuidance = "Follow standard security practices. Update affected software to the latest versions when available. Monitor vendor advisories for patches and workarounds.";
  ...
  if (mitreCveData) {
    ...
    if (mitreCveData.primaryMetric) {
      cvssScore = mitreCveData.formattedCvssScore || mitreCveData.primaryMetric.baseScore || "Unknown";
      cvssVector = mitreCveData.formattedCvssVector || mitreCveData.primaryMetric.vectorString || "Unknown";
      severityRating = mitreCveData.formattedSeverity || mitreCveData.primaryMetric.baseSeverity || "Unknown";
  ...
  if (cvssScore !== "Unknown" && cvssVector !== "Unknown") {
    ...
    if (cweId !== "Unknown") {
      ...
      if (cweId.includes("CWE-79")) {
        technicalDetails += "cross-site scripting (XSS), allowing attackers to inject malicious scripts into web pages viewed by other users.";
      } else if (cweId.includes("CWE-89")) {
        technicalDetails += "SQL injection, allowing attackers to interfere with database queries and potentially access, modify, or delete data.";
      } else if (cweId.includes("CWE-20")) {
        technicalDetails += "improper input validation, potentially allowing attackers to supply crafted input that could manipulate the application in unintended ways.";
      } else if (cweId.includes("CWE-200")) {
        technicalDetails += "information exposure or disclosure, potentially revealing sensitive data to unauthorized parties.";
      } else if (cweId.includes("CWE-22")) {
        technicalDetails += "path traversal, potentially allowing attackers to access files and directories outside of intended boundaries.";
      } else if (cweId.includes("CWE-78")) {
        technicalDetails += "OS command injection, potentially allowing attackers to execute arbitrary commands on the host operating system.";
      } else if (cweId.includes("CWE-352")) {
        technicalDetails += "cross-site request forgery (CSRF), potentially allowing attackers to trick users into performing unintended actions.";
      } else if (cweId.includes("CWE-287")) {
        technicalDetails += "improper authentication, potentially allowing attackers to bypass authentication mechanisms.";
      } else if (cweId.includes("CWE-502")) {
        technicalDetails += "deserialization of untrusted data, potentially allowing attackers to execute arbitrary code.";
      } else {
  ...
  let impactAnalysis = "Impact analysis could not be determined due to limited information.";
  if (severityRating !== "Unknown") {
    ...
    if (severityRating.toUpperCase() === "CRITICAL") {
      impactAnalysis += "Critical vulnerabilities typically represent severe security issues that require immediate attention. Exploitation could lead to complete system compromise, unauthorized access to sensitive data, or service disruption.";
    } else if (severityRating.toUpperCase() === "HIGH") {
      impactAnalysis += "High severity vulnerabilities represent significant security issues that should be prioritized for remediation. Exploitation could lead to significant data loss, system compromise, or service disruption.";
    } else if (severityRating.toUpperCase() === "MEDIUM") {
      impactAnalysis += "Medium severity vulnerabilities represent moderate security issues that should be addressed in a timely manner. Exploitation might be more difficult or limited in impact compared to higher severity issues.";
    } else if (severityRating.toUpperCase() === "LOW") {
      impactAnalysis += "Low severity vulnerabilities represent minor security issues with limited potential impact. While less urgent, they should still be addressed as part of routine security maintenance.";
    }
  }
  ...
  return {
    ...
    MITIGATION_GUIDANCE: mitigationGuidance,
    REFERENCE_URLS: references.length > 0 ? 
                  (references.length > 10 ? references.slice(0, 10).join("\n") : references.join("\n")) : 
...
async function enhanceWithRAG(inputData) {
  ...
  try {
    ...
    console.log(`Loaded index with ${index.vulnerabilities.length} entries from ${index.metadata.timestamp}`);
    ...
    if (targetCwe) {
      ...
      if (cweSimilar.length > 0) {
        console.log(`Found ${cweSimilar.length} similar vulnerabilities with matching CWE`);
        relevantVulnerabilities.push(...cweSimilar);
      }
    }
    
    // If we don't have enough, find by affected products
    if (relevantVulnerabilities.length < 4 && inputData.AFFECTED_SOFTWARE !== "Unknown") {
      ...
      if (productSimilar.length > 0) {
        console.log(`Found ${productSimilar.length} similar vulnerabilities affecting the same product`);
    ...
    if (relevantVulnerabilities.length < 4 && inputData.SEVERITY_RATING !== "Unknown") {
      ...
      if (severitySimilar.length > 0) {
        console.log(`Found ${severitySimilar.length} similar vulnerabilities with ${severity} severity`);
        // Filter out any duplicates
    ...
    console.log(`Enhancing input data with ${limitedRelevant.length} relevant vulnerabilities`);
    
    const relevantContext = limitedRelevant.map(v => {
      return `
CVE: ${v.id}
...
async function generateBlogPost(inputData) {
  ...
  try {
    ...
    console.log(`Generating content with ${tokenUsage.provider} model: ${modelOptions.model}`);
