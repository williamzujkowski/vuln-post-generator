...
async function fetchRecentVulnerabilities() {
  ...
  console.log(`Fetching vulnerabilities from ${pubStartDate} to ${pubEndDate} with CVSS >= ${minCvssScore}`);
    ...
    if (response.data && response.data.vulnerabilities) {
      ...
      const filteredVulnerabilities = response.data.vulnerabilities.filter(vuln => {
        ...
        if (metrics.cvssMetricV31 && metrics.cvssMetricV31.length > 0) {
          highestScore = Math.max(highestScore, metrics.cvssMetricV31[0].cvssData.baseScore);
        }
        if (metrics.cvssMetricV30 && metrics.cvssMetricV30.length > 0) {
          highestScore = Math.max(highestScore, metrics.cvssMetricV30[0].cvssData.baseScore);
        }
        if (metrics.cvssMetricV2 && metrics.cvssMetricV2.length > 0) {
          highestScore = Math.max(highestScore, metrics.cvssMetricV2[0].cvssData.baseScore);
        }
        
        return highestScore >= minCvssScore;
      });
      
      console.log(`Found ${filteredVulnerabilities.length} vulnerabilities with CVSS >= ${minCvssScore}`);
...
async function updateIndex() {
  ...
  if (shouldRebuild) {
    ...
    const newEntries = processedVulnerabilities.filter(entry => !existingIds.has(entry.id));
    ...
    console.log(`Updated index with ${newEntries.length} new entries, total: ${newIndex.length}`);
  }
